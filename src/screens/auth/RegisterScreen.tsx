...
const makeStyles = (c: AppColors) => StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: c.border, borderRadius: 10, padding: 14, fontSize: 15, color: c.text, backgroundColor: c.input },
...
export default function RegisterScreen({ navigation }: Props) {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const data = await register({ first_name: firstName.trim(), last_name: lastName.trim(), email: email.trim(), password, password_confirmation: confirm, role: 'publisher' });
      if (data.token && data.user) {
        await setAuth(data.token, data.user);
      } else {
        Alert.alert('Account Created', data.message ?? 'Check your email to verify your account.', [{ text: 'OK', onPress: () => navigation.navigate('Login') }]);
      }
    } catch (err: any) {
